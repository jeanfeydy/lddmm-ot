...
def ShowTransport( Q, Xt, Gamma, ax ) :
	...
	Q_points,Q_weights,_ = Q.to_measure()  ;  xtpoints = Xt.points # Extract the centers + areas
...
def DisplayShoot(Q0, G0, p0, Q1, G1, Xt, info, it, scale_momentum, scale_attach, form='.png', framenumber=0, nframes = 1) :
	...
	for fn in range(framenumber, framenumber + nframes) :
		fig.savefig( 'output/sinkhorn_varifold/iteration_' + str(fn).zfill(5) + form, bbox_inches='tight')
...
class Curve :
	...
	def to_measure(self) :
		...
		orientations = [ (seg[1] - seg[0]) / l             for (seg, l) in zip(segments, lengths)]
		return ( np.array(centers), np.array(lengths), np.array(orientations) )
...
def ot_matching(q1_x, q1_mu, q1_u, xt_x, xt_mu, xt_u, tau = -.8) :
...
def perform_matching( Q0, Xt, params, scale_momentum = 1, scale_attach = 1) :
	...
	(Xt_x, Xt_mu, Xt_u) = Xt.to_measure()      # Transform the target into a measure once and for all
	(Q0_x, Q0_mu, Q0_u) = Q0.to_measure()
	q0 = Q0.points ; p0 = np.zeros(q0.shape)    # Null initialization for the shooting momentum
	
	[gammas, costs, errs] = ot_matching(Q0_x, Q0_mu, Q0_u, Xt_x, Xt_mu, Xt_u, tau = -.8)
	...
	np.savetxt('output/sinkhorn_varifold/costs_errs.csv', costs_errs)
