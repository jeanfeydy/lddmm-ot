...
__global__ void GaussGpuGrad1ConvOnDevice (TYPE ooSigma2, TYPE *alpha, TYPE *x, TYPE *beta, TYPE *gamma, int nx)
{
	...
	TYPE xi[DIMPOINT], xmx[DIMPOINT], alphai[DIMPOINT],  gammai[DIMPOINT];
	...
	for(int jstart = 0, tile = 0; jstart < nx; jstart += blockDim.x, tile++)
	{
		...
		if(i<nx) // we compute gammai only if i is in the range
		{
			...
			for(int jrel = 0; jrel < blockDim.x && jrel<nx-jstart; jrel++, xj+=inc, betaj+=inc, alphaj+=inc)
			{
				...
				TYPE s =  (-ooSigma2*2.0f) * exp(-r2*ooSigma2);
				/*TYPE s =  2.0f * drhox(xi,xj,ooSigma2,DIMPOINT);*/
				...
				for(int k=0; k<DIMPOINT; k++)
				{
					...
					{
						/* gammai[k] +=  s * betaj[k] * (xi[l]-xj[l]) * (alphai[l] - alphaj[l]); */
	...
	{ 
...
#if UseCudaOnDoubles
		GaussGpuGrad1Conv_double(oosigma2,alpha,x,beta,gamma,dimpoint,nx);  
#else
		// convert to float
		...
		// function calls;
		GaussGpuGrad1Conv_float(oosigma2,alpha_f,x_f,beta_f,gamma_f,dimpoint,nx);
