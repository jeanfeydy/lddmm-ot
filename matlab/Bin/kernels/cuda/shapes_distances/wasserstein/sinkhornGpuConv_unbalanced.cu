...
int SinkhornGpuStep(TYPE epsilon,TYPE lambda, TYPE rho, TYPE weightGeom,TYPE weightGrass,
		...
		int dimPoint, int dimVect, int nx, int ny, int max_iter)
{
	...
	TYPE KLdumu = 0.0f, KLdvnu = 0.0f, totalmass = 0.0f;
	for (int i=0;i<nx;i++)
		KLdumu += (exp(- gammax_h[i]/rho ) -1) * mu_h[i];

	for (int j=0;j<ny;j++)
		KLdvnu += (exp(- gammay_h[j]/rho ) -1) * nu_h[j];
	...
	// compute the energy
	*gammaWd  = totalmass - rho * KLdvnu - rho *  KLdumu ;
...
{ 
	...
	if( !mxIsDouble(prhs[argu]) || mxIsComplex(prhs[argu]) ||
			mxGetN(prhs[argu])*mxGetM(prhs[argu])!=1 ) {
		mexErrMsgTxt("Input rho must be a scalar.");
	}
	/*  get the scalar input epsilon */
	double rho = mxGetScalar(prhs[argu]);
	if (rho <= 0.0)
		mexErrMsgTxt("Input rho must be a positive number.");
...
#if UseCudaOnDoubles
	SinkhornGpuStep<double>(epsilon,lambda,rho,weightG[0],weightG[1],alpha,x,y,beta,mu,nu,gammax,gammay,gammaWd,dimpoint,dimvect,nx,ny,max_iter); 
#else
	// convert to float
	...
	// function calls;
	SinkhornGpuStep<float>(epsilon,lambda,rho,weightG[0],weightG[1],alpha_f,x_f,y_f,beta_f,mu_f,nu_f,gammax_f,gammay_f,&gammaWd_f,dimpoint,dimvect,nx,ny,max_iter);
