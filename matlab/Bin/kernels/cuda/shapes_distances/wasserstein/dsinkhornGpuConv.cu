...
int SinkhornGpuGrad(TYPE epsilon,TYPE lambda, TYPE weightGeom,TYPE weightGrass,
		TYPE* alpha_h, TYPE* x_h, TYPE* y_h, TYPE* beta_h, TYPE* mu_h, TYPE* nu_h, TYPE* gammax_h, TYPE* gammadx_h,
		int dimPoint, int dimVect, int nx, int ny, int max_iter)
{
	...
	cudaMalloc((void**)&gammadx_d, sizeof(TYPE)*(nx*dimPoint));
	...
	if(dimPoint==2 && dimVect==1)
		GradxWOnDevice<TYPE,2,1><<<gridSizex,blockSizex,blockSizex.x*(dimPoint+dimVect)*sizeof(TYPE)>>>
			(epsilon,weightGeom,weightGrass,alpha_d, x_d, y_d, beta_d, gammadx_d, nx, ny);
	else if(dimPoint==4 && dimVect==1)
		GradxWOnDevice<TYPE,4,1><<<gridSizex,blockSizex,blockSizex.x*(dimPoint+dimVect)*sizeof(TYPE)>>>
			(epsilon,weightGeom,weightGrass,alpha_d, x_d, y_d, beta_d, gammadx_d, nx, ny);
	else if(dimPoint==6 && dimVect==1)
		GradxWOnDevice<TYPE,6,1><<<gridSizex,blockSizex,blockSizex.x*(dimPoint+dimVect)*sizeof(TYPE)>>>
			(epsilon,weightGeom,weightGrass,alpha_d, x_d, y_d, beta_d, gammadx_d, nx, ny);
	else
	{
		printf("error: dimensions of GradxWOnDevice kernel not implemented in cuda");
		cudaFree(alpha_d);
		cudaFree(x_d);
		cudaFree(y_d);
		cudaFree(beta_d);
	...
	// Send data from device to host.
	cudaMemcpy(gammadx_h,gammadx_d,  sizeof(TYPE)*(nx*dimPoint),cudaMemcpyDeviceToHost);
...
{ 
	...
	/*  set the output pointer to the output result(vector) */
	plhs[1] = mxCreateDoubleMatrix(dimpoint,nx,mxREAL);
	/*  create a C pointer to a copy of the output result(vector)*/
	double *gammaDx = mxGetPr(plhs[1]);


#if UseCudaOnDoubles
	SinkhornGpuGrad<double>(epsilon,lambda,weightG[0],weightG[1],alpha,x,y,beta,mu,nu,gammax,gammaDx,dimpoint,dimvect,nx,ny,max_iter); 
#else
	// convert to float
	...
	float *gammaDx_f = new float[nx*dimpoint];
	...
	// function calls;
	SinkhornGpuGrad<float>(epsilon,lambda,weightG[0],weightG[1],alpha_f,x_f,y_f,beta_f,mu_f,nu_f,gammax_f,gammaDx_f,dimpoint,dimvect,nx,ny,max_iter);

	for(int i=0; i<nx; i++)
		gammax[i] = gammax_f[i];

	for(int i=0; i<nx*dimpoint; i++)
		gammaDx[i] = gammaDx_f[i];
